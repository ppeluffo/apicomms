...
class DlgConfigAllFrameUsecase:
    ...
    def procesar_frame(self, d_params=None):
        ...
        if bd_Bhash != int(d_params.get('BH','-1'),16):
            ...
            new_conf = True
            if current_app.config["UNIT_ID"] == current_app.config["DEBUG_ID"]:
                self.logger.info(f"ID={current_app.config['UNIT_ID']}: Config BASE")
        ...
        if bd_Ahash != int(d_params.get('AH','-1'),16):
            ...
            new_conf = True
            if current_app.config["UNIT_ID"] == current_app.config["DEBUG_ID"]:
                self.logger.info(f"ID={current_app.config['UNIT_ID']}: Config ANALOG")
        ...
        if bd_Chash != int(d_params.get('CH','-1'),16):
            ...
            new_conf = True
            if current_app.config["UNIT_ID"] == current_app.config["DEBUG_ID"]:
                self.logger.info(f"ID={current_app.config['UNIT_ID']}: Config COUNTER")
        ...
        if bd_Mhash != int(d_params.get('MH','-1'),16):
            ...
            new_conf = True
            if current_app.config["UNIT_ID"] == current_app.config["DEBUG_ID"]:
                self.logger.info(f"ID={current_app.config['UNIT_ID']}: Config MODBUS")
        ...
        if bd_Phash != int(d_params.get('PH','-1'),16):
            ...
            new_conf = True
            if current_app.config["UNIT_ID"] == current_app.config["DEBUG_ID"]:
                self.logger.info(f"ID={current_app.config['UNIT_ID']}: Config PRESION")
        ...
        if bd_Fhash != int(d_params.get('FH','-1'),16):
            ...
            new_conf = True
            if current_app.config["UNIT_ID"] == current_app.config["DEBUG_ID"]:
                self.logger.info(f"ID={current_app.config['UNIT_ID']}: Config FLOWC")
